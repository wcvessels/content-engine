# Stage 04: Render (educational-guide)

One doc-model in, the enabled targets out (PDF + HTML for this submission). This stage is GENERIC: it reads only the doc-model + `_config` + the shared renderer and never needs to know which lane produced the model.
...
4. HTML (build first, the spine), single self-contained file: inline CSS, base64 screenshots (crop applied with Pillow before downscale per the block's `crop`, original frame untouched), sticky `<nav>` TOC with plain in-page anchor links, collapsible `<details open>`, restrained ~150ms transitions only.
...
6. DOCX: NOT built this submission. The shared `render.py` carries a documented `build_docx` stub; `../../../../_skills/docx/SKILL.md` describes the pandoc + reference-doc path.
...
- Images embedded: no broken links; crops applied as specified; original frames in `../01-ingest/output/ingest/frames/` untouched.
