...
You are in the **sop** lane: a self-contained ICM workspace that turns a recorded process walkthrough into a step-by-step Standard Operating Procedure (numbered actions, owners where named, decisions, risks, and a screenshot at the step it illustrates). This is content-engine's SECOND lane. It shares the entire shelf with educational-guide and specializes ONLY at stages 02 and 03.
...
- **Lane dials:** `lane-config.md` (`doc_type=procedure`, default reader, significance thresholds).
- **How this lane structures:** `reference/rules.md` (the zero-inference thesis + the S1-S5 delta vs educational) + `reference/examples.md` (a worked Procedure section). The loadable detail lives in the two stage `references/`.
...
This lane SPECIALIZES only at stages **02 (extract)** and **03 (layout)**. Stages 01 / 04 / 05 are generic: they obey the canonical `_standards/` contracts and invoke the shared `_skills/render-doc` code, carrying no lane-specific logic. The deepest difference from educational is the inference posture: an SOP asserts facts, so it infers NOTHING on content (educational may infer order).

## What-to-Load / What-NOT-to-Load (THE canonical load-rule table)

Five-layer routing + the 60/30/10 build ratio (taught-ICM: 60% deterministic infra, 30% orchestration, 10% AI judgment): most stages load deterministic `_config`/references; AI judgment is the thin layer at 02/03 only. `[SPEC]` notes are the only per-lane differences. Paths are relative to a stage folder (`stages/NN-name/`).
